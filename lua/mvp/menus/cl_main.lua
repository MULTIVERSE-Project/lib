...
local function createQuickActions(actionsPnl, title, buttons)
    ...
    function base:Paint(w, h)
        ...
        draw.SimpleText(title, mvp.Font(16), 0, 3, mvp.Color('primary_text'))
    end
...
local function createHomeContent(content)
    ...
    function welcome:Paint(w, h)
        ...
        draw.SimpleText(mvp.Lang('menuWelcomeText1', LocalPlayer():Name()), mvp.Font(28), 90, y1 + 8, mvp.Color('primary_text'), nil, TEXT_ALIGN_CENTER)
        draw.SimpleText(mvp.Lang('menuWelcomeText2'), mvp.Font(18), 90, y2 + 8, mvp.Color('primary_text'), nil, TEXT_ALIGN_CENTER)
    end
    ...
    function actionsMenus:Paint(w, h)
        ...
        draw.SimpleText(mvp.Lang('menuQuickActions'), mvp.Font(18), 10 + 5, 15, mvp.Color('primary_text'), nil, TEXT_ALIGN_CENTER)
    end
                ...
                function changelog:Paint(w, h)
                    ...
                    for k, info in ipairs(v.data.blocks) do
                        if info.type == 'paragraph' then
                            ...
                            draw.DrawText(info.text, mvp.Font(18, 500), 15, y, mvp.Color('primary_text'))
                            y = y + th
                        end

                        if info.type == 'header' then
                            local _, th = draw.SimpleText(info.text, mvp.Font(sizesMap[info.data.level], 500), 10, y, mvp.Color('primary_text'))
                            y = y + th
                        end
                    end
