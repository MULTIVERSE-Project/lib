...
concommand.Add('mvp_ui_showcase', function()
    ...
    sidebar:AddButton('circle-question', 'About', function()
        ...
        function credits:Paint(w, h)
            ...
            draw.SimpleText('MULTIVERSE Library', mvp.utils.GetFont(28, 'Proxima Nova Rg', 500), w * .5 - 100, h * .5 - 10, theme:GetColor('accent'), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
            draw.SimpleText('created by Kot', mvp.utils.GetFont(24, 'Proxima Nova Rg', 500), w * .5 - 115, h * .5 + 10, theme:GetColor('text_primary'), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)

            draw.SimpleText('MULTIVERSE Project', mvp.utils.GetFont(28, 'Proxima Nova Rg', 500), w * .5 + 100, h * .5 - 10, theme:GetColor('accent'), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
            draw.SimpleText('created by Kot & Ben_Bro', mvp.utils.GetFont(24, 'Proxima Nova Rg', 500), w * .5 + 115, h * .5 + 10, theme:GetColor('text_primary'), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        end
        ...
        function changelogs:Paint(w, h)
            ...
            surface.SetDrawColor(theme:GetColor('secondary_text'))
            draw.NoTexture()
        ...
        http.Fetch('https://multiverse-project.com/api/v1/changelogs/all', function(data, _, _, status)
            ...
            for k, v in SortedPairsByMemberValue(response, 'created_at', true) do
                ...
                function changelog:Paint(w, h)
                    ...
                    local _, th = draw.SimpleText(v.title, mvp.utils.GetFont(32, 'Proxima Nova Rg', 500), 10, y, theme:GetColor('accent_text'))
                    y = y + th
                    local _, th = draw.SimpleText(timePublised, mvp.utils.GetFont(16, 'Proxima Nova Rg', 500), 10, y, theme:GetColor('secondary_text'))
                    y = y + th + 5
                    ...
                    for k, info in ipairs(v.data.blocks) do
                        if info.type == 'paragraph' then
                            ...
                            draw.DrawText(info.text, mvp.utils.GetFont(18, 'Proxima Nova Rg', 500), 15, y, theme:GetColor('text'))
                            y = y + th
                        end

                        if info.type == 'header' then
                            local _, th = draw.SimpleText(info.text, mvp.utils.GetFont(sizesMap[info.data.level], 'Proxima Nova Rg', 500), 10, y, theme:GetColor('text'))
                            y = y + th
                        end
                    end
